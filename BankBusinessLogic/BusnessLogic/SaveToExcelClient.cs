          ...
          SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.Workbook.AppendChild<Sheets>(new Sheets());
                ...
                foreach (var order in info.Deals)
                {
                    if (!deals.Contains(order.DealName))
                    {
                        deals.Add(order.DealName);
                    }
                }
                ...
                foreach (var deal in deals)
                {
                    ...
                    foreach (var credit in info.Deals.Where(rec => rec.DealName == deal))
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
                        ...
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            ...
                            ColumnName = "C",
                            RowIndex = rowIndex,
                            Text = Convert.ToString(credit.date),
                            StyleIndex = 1U
                        });
                        rowIndex++;
                    }
