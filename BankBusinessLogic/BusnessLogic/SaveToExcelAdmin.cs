...
{
    public static class SaveToExcelAdmin
    {
          ...
          SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
               ...
               spreadsheetDocument.WorkbookPart.Workbook.AppendChild<Sheets>(new Sheets());
                ...
                foreach (var currency in info.Request.MoneyCount)
                {
                    if (!money.ContainsKey(currency.Key))
                    {
                        money.Add(currency.Key, currency.Value);
                    }
                }
